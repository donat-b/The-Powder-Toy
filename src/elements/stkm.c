...
int run_stickman(playerst* playerp, UPDATE_FUNC_ARGS) {
	...
	if (INBOND(playerp->legs[4], playerp->legs[5]) && !eval_move(PT_DUST, (int)playerp->legs[4], (int)playerp->legs[5], NULL))
	{
		playerp->legs[4] = playerp->legs[6];
		playerp->legs[5] = playerp->legs[7];
	}

	if (INBOND(playerp->legs[12], playerp->legs[13]) && !eval_move(PT_DUST, (int)playerp->legs[12], (int)playerp->legs[13], NULL))
	{
