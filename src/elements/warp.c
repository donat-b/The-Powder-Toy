...
int update_WARP(UPDATE_FUNC_ARGS) {
	...
	for ( trade = 0; trade<5; trade ++)
	{
		...
		if (x+rx>=0 && y+ry>0 && x+rx<XRES && y+ry<YRES && (rx || ry))
		{
			...
			if (!r)
				continue;
			if ((r&0xFF)!=PT_WARP&&(r&0xFF)!=PT_STKM&&(r&0xFF)!=PT_STKM2&&!(ptypes[r&0xFF].properties&PROP_INDESTRUCTIBLE)&&!(ptypes[r&0xFF].properties&PROP_CLONE)&&(r&0xFF)!=PT_BCLN&&(10>=rand()%200))
			{
